<template>
  <menu>
    <li
      ...
      :key="menu.id"
      :class="[
        'flex  h-[34px] items-center  mx-6 my-1 py-3 px-4 rounded-3 justify-between',
        activeMenuClass(menu),
        'cursor-pointer hover:bg-light-2',
        isHover ? 'bg-light-2' : '',
        'transition-colors group'
      ]"
      @click="navigator(menu)"
    >
...
const props = withDefaults(defineProps<{ menus?: Menu[]; type?: 'menu' | 'category' }>(), {
  menus: undefined,
  type: 'menu'
})
